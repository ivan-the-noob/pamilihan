...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['tcat_name'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM tbl_top_category WHERE seller_id=? and tcat_name=? and tcat_name!=?");
    	$statement->execute(array($myId, $_POST['tcat_name'],$current_tcat_name));
    ...
    if($valid == 1) {    	
		...
		$statement = $pdo->prepare("UPDATE tbl_top_category SET seller_id=?, tcat_name=?,show_on_menu=? WHERE tcat_id=?");
		$statement->execute(array($myId,$_POST['tcat_name'],$_POST['show_on_menu'],$_REQUEST['id']));
