        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	...
            	$i=0;
                if($_SESSION['user']['role'] == "Admin"){
                    $statement = $pdo->prepare("SELECT t1.seller_id as selId, t1.*, t2.*
                ...
                if($_SESSION['user']['role'] == "Seller"){
                    $statement = $pdo->prepare("SELECT t1.seller_id as selId, t1.*, t2.*
                                    ...
                                    ON t1.tcat_id = t2.tcat_id WHERE t1.seller_id=?
                                    ORDER BY t1.mcat_id DESC");
            	...
            	foreach ($result as $row) {
					...
					<tr>
                        ...
                        if(isset($_SESSION['user'])){
                            if($_SESSION['user']['role'] == "Admin"){
                                $sql = $pdo->prepare("SELECT * FROM tbl_seller WHERE user_id=?");
                                $sql->execute(array($row['selId']));
